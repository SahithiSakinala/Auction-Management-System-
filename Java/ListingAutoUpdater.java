 ...
 * 
 * A server side thread that periodically does checks for all auction bids.
...
class ListingAutoUpdater extends Thread {
	...
	/*
	 * Go through the current set of auto bids and check if they have been out bidded.
	 * In that case, go to each relevant listing and attempt to outbid the current max as
	 * long as we do not violate the maximum limit set for the autobid.
	 */
